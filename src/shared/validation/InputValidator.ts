...
export class InputValidator {
   ...
   */
  static validateJson(input: string, maxDepth: number = 10): ValidationResult & { data?: any } {
    if (typeof input !== 'string') {
...
export const Validators = {
  ...
  serviceUrl: (input: string): ValidationResult =>
    InputValidator.validateUrl(input, {
      allowedProtocols: ['http:', 'https:'],
      allowLocalhost: process.env['NODE_ENV'] === 'development',
      allowPrivateIps: process.env['NODE_ENV'] === 'development',
      requireHttps: false,
   ...
   */
  exportData: (input: string): ValidationResult & { data?: any } =>
    InputValidator.validateJson(input, 20), // Allow deeper nesting for complex configs
