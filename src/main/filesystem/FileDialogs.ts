...
interface FileDialogsEvents {
  'dialog-opened': { type: DialogType; title: string };
  'dialog-closed': { type: DialogType; result: FileSelectionResult | DirectorySelectionResult };
  'files-selected': { filePaths: string[]; type: DialogType };
  'files-validated': { results: FileValidationResult[] };
  'security-warning': { filePath: string; threats: string[] };
  'recent-files-updated': { entries: RecentFileEntry[] };
}
 ...
 */
export class FileDialogs extends EventEmitter {
  private readonly maxFileSize: number = 100 * 1024 * 1024; // 100MB
  private readonly maxFiles: number = 10;
  private readonly recentFiles: Map<string, RecentFileEntry> = new Map();
  private readonly recentFilesLimit: number = 20;
   ...
   */
  private createFilter(filterType: keyof typeof this.filterDefinitions): FileFilter {
  ...
  public async showImportDialog(
    ...
    const dialogOptions: FileDialogOptions = {
      type: 'import',
   ...
   */
  public async showBackupDirectoryDialog(
    parentWindow?: BrowserWindow
  ): Promise<DirectorySelectionResult> {
    this.emit('dialog-opened', { type: 'backup', title: 'Select Backup Directory' });

    try {
      const result = await dialog.showOpenDialog(parentWindow || this.getActiveWindow(), {
      ...
      if (result.canceled || !result.filePaths.length) {
        const cancelResult: DirectorySelectionResult = {
          success: false,
          directoryPath: '',
          cancelled: true,
          errors: [],
      ...
      const finalResult: DirectorySelectionResult = {
      ...
      const errorResult: DirectorySelectionResult = {
        success: false,
        directoryPath: '',
        cancelled: false,
   ...
   */
  public async validateDroppedFiles(filePaths: string[]): Promise<FileSelectionResult> {
    ...
    for (const filePath of filePaths) {
      try {
        ...
        result.metadata.invalidFiles++;
        result.errors.push(
          `Error validating ${path.basename(filePath)}: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    }
   ...
   */
  public getRecentFiles(fileType?: string): RecentFileEntry[] {
   ...
   */
  public async openFileInSystem(filePath: string): Promise<boolean> {
    try {
   ...
   */
  public async showFileInDirectory(filePath: string): Promise<boolean> {
    try {
  ...
  private async showFileDialog(
    ...
    try {
      const dialogResult = await dialog.showOpenDialog(parentWindow || this.getActiveWindow(), {
        title: options.title,
      ...
      const errorResult: FileSelectionResult = {
        success: false,
        filePaths: [],
        cancelled: false,
  ...
  private async showSaveDialog(
    ...
    try {
      const dialogResult = await dialog.showSaveDialog(parentWindow || this.getActiveWindow(), {
        title: options.title,
   ...
   * Validate selected files
   */
  private async validateSelectedFiles(
    filePaths: string[],
    options: FileDialogOptions
  ): Promise<FileSelectionResult> {
    const result: FileSelectionResult = {
   ...
   * Validate save path
   */
  private async validateSavePath(
    savePath: string
   ...
   * Validate directory
   */
  private async validateDirectory(directoryPath: string): Promise<{
    ...
    try {
      ...
      errors.push(
        `Directory validation error: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
   ...
   */
  private validateFilePath(filePath: string): { safe: boolean; threats: string[] } {
    const threats: string[] = [];
    const platform = process.platform as keyof typeof this.platformConfig;
   ...
   */
  private getDefaultPath(type: DialogType): string {
    switch (type) {
      case 'import':
   ...
   */
  private getActiveWindow(): BrowserWindow {
    const windows = BrowserWindow.getAllWindows();
    return windows.find(win => win.isFocused()) || windows[0] || new BrowserWindow();
  }

  /**
   * Add file to recent files list
   */
  private async addToRecentFiles(filePath: string, type: string): Promise<void> {
    try {
      const stats = await fs.stat(filePath);
  ...
  public override emit<K extends keyof FileDialogsEvents>(
    event: K,
    ...args: [FileDialogsEvents[K]]
  ): boolean {
    return super.emit(event, ...args);
  }

  public override on<K extends keyof FileDialogsEvents>(
    event: K,
    listener: (arg: FileDialogsEvents[K]) => void
  ): this {
    return super.on(event, listener);
  }

  public override once<K extends keyof FileDialogsEvents>(
    event: K,
    listener: (arg: FileDialogsEvents[K]) => void
  ): this {
